...
#     U1[-1] = U0[-1] - alpha*(U0[-1] - U0[-2])
#     return U1

# def schema2(U0,J,alpha):
#     U1 = np.zeros(J)
#     U1[1:-1] = U0[1:-1] - alpha/2.*(U0[2:] - U0[0:-2]) + (alpha**2)/2.*(U0[2:] - 2.*U0[1:-1] + U0[:-2])
...
#     U1 = np.zeros(J)
#     U1[1:J-1] = U0[1:J-1] - alpha*(U0[1:J-1] - U0[1:J-3]) - alpha*(1-alpha)/4.*(U0[3:J-1]-U0[2:J-2]-U0[1:J-3]+U0[0:J-4])
#     U1[0] = U0[0] - alpha*(U0[0] - U0[-1]) - alpha*(1-alpha)/4.*(U0[1]-U0[0]-U0[J-2]+U0[J-3])
#     U1[1] = U0[1] - alpha*(U0[1] - U0[0]) - alpha*(1-alpha)/4.*(U0[2]-U0[1]-U0[0]+U0[J-2])
#     U1[J-2] = U0[J-2] - alpha*(U0[J-2] - U0[J-3]) - alpha*(1-alpha)/4.*(U0[J-1]-U0[J-2]-U0[J-3]+U0[J-4])
#     U1[J-1] = U0[J-1] - alpha*(U0[J-1] - U0[J-2]) - alpha*(1-alpha)/4.*(U0[0]-U0[J-1]-U0[J-2]+U0[J-3])
#     return U1

def schema1(U0,J,alpha):
    U1 = np.zeros(J)
    U1[1:] = U0[1:] - alpha*(U0[1:] - U0[0:-1])
    U1[0] = U0[0] - alpha*(U0[0] - U0[-1])
    return U1

def schema2(U0,J,alpha):
    U1 = np.zeros(J)
    U1[1:-1] = U0[1:-1] - alpha/2.*(U0[2:] - U0[0:-2]) + (alpha**2)/2.*(U0[2:] - 2.*U0[1:-1] + U0[0:-2])
    U1[0] = U0[0] - alpha/2.*(U0[1] - U0[-1]) + (alpha**2)/2.*(U0[1] - 2.*U0[0] + U0[-1])
    U1[-1] = U0[-1] - alpha/2.*(U0[0] - U0[-2]) + (alpha**2)/2.*(U0[0] - 2.*U0[-1] + U0[-2])
    return U1
...
def schema3(U0,J,alpha):
    U1 = np.zeros(J)
    U1[2:-1] = U0[2:-1] - alpha*(U0[2:-1] - U0[1:-2]) - alpha*(1-alpha)/4.*(U0[3:]-U0[2:-1]-U0[1:-2]+U0[0:-3])
    U1[0] = U0[0] - alpha*(U0[0] - U0[-1]) - alpha*(1-alpha)/4.*(U0[1]-U0[0]-U0[-1]+U0[-2])
    U1[1] = U0[1] - alpha*(U0[1] - U0[0]) - alpha*(1-alpha)/4.*(U0[2]-U0[1]-U0[0]+U0[-1])
...
def E3(xi, alpha): 
    theta = xi/2.
    return 1-np.sqrt((1-2*alpha*(np.sin(theta)**2)*(1-(1-alpha)*(np.cos(theta)**2)))+ 4*(alpha**2)*(np.cos(theta)**2)*(1-(1-alpha)*(np.sin(theta)**2)))

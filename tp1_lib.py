...
def Ep3(xi, alpha):
	theta = xi/2.
	num = 1-2*alpha*((np.sin(theta))**2)*(1-(1-alpha)*((np.cos(theta))**2))
	denum2 = num**2 + 4*(alpha**2)*((np.sin(theta))**2)*((np.cos(theta))**2)*((1+(1-alpha)*((np.sin(theta))**2))**2)
	return alpha*xi - np.arccos(num/np.sqrt(denum2))    